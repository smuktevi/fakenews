...
def read_edges():
    ...
    with open('userarticle_tweets.json', 'w') as fp:
        json.dump(userarticle_tweets, fp)


def generate_edges_based_on_timestamp():
    with open('tweet_content.json') as json_tweets:
...
def generate_aliases_for_userids():
    ...
    with open('aliasval_userid.json', 'w') as fp:
        json.dump(aliasval_userid, fp)


def generate_bigraph():
    with open('user_firsttweet_article.json', mode='w') as edgesfile:
    ...
    with open('userid_aliasval.json') as json_useralias:
        user_alias = json.load(json_useralias)

    with open('user_article.bigraph', mode='w') as bigraph:
        for userarticle in userarticle_tweet.keys():
            user_article = userarticle.split(",")
            bigraph.write(user_article[1].replace("politifact", "") + " " + str(user_alias[user_article[0]]) + "\n")

    with open('user_article_raw.bigraph', mode='w') as bigraph:
        for userarticle in userarticle_tweet.keys():
            user_article = userarticle.split(",")
