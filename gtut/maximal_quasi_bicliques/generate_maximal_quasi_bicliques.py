...
def get_neighbours(vertex, given_set):
    ...
    # common_neighbours = list(set(vertex_neighbours) & set(given_set))
    #
...
def get_pairwise_neighbours(vertex1, vertex2, given_set):
    # vertex1_neighbours = list(graph.neighbors(vertex1))
    # vertex2_neighbours = list(graph.neighbors(vertex2))
    # common_neighbours_1 = list(set(vertex1_neighbours) & set(given_set))
    # common_neighbours_2 = list(set(vertex2_neighbours) & set(given_set))
    # common_neighbours = list(set(common_neighbours_1) & set(common_neighbours_2))
    #
    # vertex1_neighbours.clear()
    # vertex2_neighbours.clear()
    # common_neighbours_1.clear()
    # common_neighbours_2.clear()
    #
    # return common_neighbours
...
def get_maximal_quasi_biclique(x_curr, neighbours_x_curr, ms, error):
    # print("\t", "neighbours_x_curr ", "--> ", *neighbours_x_curr)
    # print("\t", "cand_exts_x_curr ", "--> ", *cand_exts_x_curr)
    ...
    if len(x_bar) > 0:
        ...
        # # find subsets which have to be validated for quasi bi-cliqueness
        # comb_numb = ms - len(y_bar)
        # if comb_numb <= 0:
        #     comb_numb = 1
        # end = len(neighbours_x_curr) - 1  # because whole n(x) is already checked against x in the first step
        #
        # while comb_numb <= end:
        #     neighbour_x_curr_subsettuples = itertools.combinations(neighbours_x_temp, comb_numb)
        #     comb_numb = comb_numb + 1
        #
        #     # validate each subset for quasi bi-cliqueness and maximality
        #     for subset in neighbour_x_curr_subsettuples:
        #         subset = list(subset)  # not every subset should be checked todo
        #         if is_quasi_biclique(x_curr, subset, error):
        #             if validate_y_maximality(x_curr, subset, neighbours_x_temp, error):
        #                 actual_subset = subset + y_bar
        #                 if validate_x_maximality(x_curr, cand_exts_x_curr, actual_subset, error):
        #                     qbc_count = qbc_count + 1
        #                     print("\t", str(qbc_count), "--> ", *x_curr, " : ", *actual_subset)
        #                     store(x_curr, actual_subset)
...
def main():
    ...
    # read_graph_edges("/Users/sivacharan/PycharmProjects/FakeNews/user_article.bigraph")
    # read_graph_edges("/Users/sivacharan/PycharmProjects/FakeNews/user_article_raw.bigraph")
    read_graph_edges('user_article_raw_alias.bigraph')
